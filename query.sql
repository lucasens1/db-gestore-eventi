...
SELECT * FROM `events` WHERE `events`.`price` IS NULL; -- (19)
...
SELECT * FROM `events` WHERE `events`.`price` < 20 AND `events`.`duration` < '03:00:00';

-- Selezionare tutti gli eventi di dicembre 2023 (25)
SELECT * FROM `events` WHERE YEAR(`events`.`start`) = 2023; 

-- Selezionare tutti gli eventi con una durata maggiore alle 2 ore (823)
SELECT * FROM `events` WHERE `events`.`duration` > '02:00:00';

-- Selezionare tutti gli eventi, mostrando nome, data di inizio, ora di inizio, ora di fine e durata totale (1040)
SELECT `events`.`name`, DATE(`events`.`start`), TIME(`events`.`start`), TIME(ADDTIME(`events`.`start`, `events`.`duration`)), `events`.`duration`  FROM `events` 
